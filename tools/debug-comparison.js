...
console.log('Testing heart puzzle...');
console.log('Original solution:');
console.log(JSON.stringify(heartPuzzle.solution, null, 2));

const result = solver.validatePuzzle(heartPuzzle.solution);
console.log('\nValidation result:');
...
if (result.solutions.length > 0) {
    console.log('\nFound solution:');
    console.log(JSON.stringify(result.solutions[0], null, 2));
    
    console.log('\nComparison:');
    console.log('Solutions match:', solver.solutionsMatch(result.solutions[0], heartPuzzle.solution));
    
    // Show differences
    console.log('\nDifferences:');
    for (let row = 0; row < heartPuzzle.solution.length; row++) {
        for (let col = 0; col < heartPuzzle.solution[row].length; col++) {
            const val1 = result.solutions[0][row][col] === solver.FILLED || result.solutions[0][row][col] === true;
            const val2 = heartPuzzle.solution[row][col] === solver.FILLED || heartPuzzle.solution[row][col] === true;
            if (val1 !== val2) {
                console.log(`Row ${row}, Col ${col}: Original=${val2}, Found=${val1}`);
            }
        }
    }
}
