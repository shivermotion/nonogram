...
/**
 * Test the nonogram solver with a few sample puzzles
 */
...
console.log('Testing Nonogram Solver...\n');

// Test a few puzzles
const testPuzzles = puzzleData.slice(0, 10); // Test first 10 puzzles

let validCount = 0;
let ambiguousCount = 0;
let invalidCount = 0;

testPuzzles.forEach((puzzle, index) => {
    console.log(`Testing ${index + 1}. ${puzzle.name} (${puzzle.id})`);
    ...
    if (result.status === 'VALID') {
        ...
        ambiguousCount++;
        console.log(`  ⚠️ Ambiguous - multiple solutions`);
    } else {
        invalidCount++;
...
console.log('=== SUMMARY ===');
console.log(`Total tested: ${testPuzzles.length}`);
console.log(`Valid: ${validCount}`);
console.log(`Ambiguous: ${ambiguousCount}`);
console.log(`Invalid: ${invalidCount}`);

if (invalidCount > 0 || ambiguousCount > 0) {
    console.log('\n⚠️ Some puzzles need attention!');
} else {
    console.log('\n✅ All tested puzzles are valid!');
}
